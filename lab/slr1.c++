...
string move_dot(const string &input)
{
    string result = input;
    size_t dot_pos = result.find('.');
    if (dot_pos != string::npos && dot_pos < result.length() - 1)
    {
        swap(result[dot_pos], result[dot_pos + 1]);
    }
    return result;
}
                ...
                for (int j = 0; j < I->prod_count; j++)
                {
                    if (I->prod[j][0] == a)
                    {
                        if (!in_state(I0, I->prod[j]))
                        {
                            ...
                            {
                                cerr << "Error: Too many productions in closure" << endl;
                                return;
                            }
                        }
                    }
...
string chars_after_dots(struct state *I)
{
    string result;
    for (int i = 0; i < I->prod_count; i++)
    {
